...
type HistoryActionStoryCreate struct {
	...
	CustomFieldValueIds []strfmt.UUID `json:"custom_field_value_ids"`
...
func (m *HistoryActionStoryCreate) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateCustomFieldValueIds(formats); err != nil {
		res = append(res, err)
	}
...
func (m *HistoryActionStoryCreate) validateCustomFieldValueIds(formats strfmt.Registry) error {
	if swag.IsZero(m.CustomFieldValueIds) { // not required
		return nil
	}

	for i := 0; i < len(m.CustomFieldValueIds); i++ {

		if err := validate.FormatOf("custom_field_value_ids"+"."+strconv.Itoa(i), "body", "uuid", m.CustomFieldValueIds[i].String(), formats); err != nil {
			return err
		}
...
func (m *HistoryActionStoryCreate) validateGroupID(formats strfmt.Registry) error {
